 ...
 validatefun();
  ...
  return (
   ...
   { loading?<HomePageSkl/>:<div className='flex justify-start items-center flex-wrap '>
    {data&&data.map((item)=>(
        <Link href={`/assignment/${item.Regdomain._id}`} key={item._id}><Card title={item.Regdomain.title} description={item.Regdomain.desc} duration={item.Regdomain.duration} validity={"1"} img={item.Regdomain.img} skills={item.Regdomain.skills} isadmin={false} progress={progress==null?0:progress} assignment="true"/></Link>
      ))}
